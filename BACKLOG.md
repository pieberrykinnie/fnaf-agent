...
- [x] `CTFMemoryStateReader` → GameState from dynamically scanned active objects. Acceptance: `auto_map.py` successfully reads FNAF state objects (power, doors, lights, etc.) live, and values are stable across two game launches. (CTF runtime traversal unit-tested offline; remaining: run `auto_map.py` live to map names/indices).
